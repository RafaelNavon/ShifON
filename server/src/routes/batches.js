...
router.post('/', async (req, res) => {
  const { bull_id, slot_id, quantity, sio_batch_code, production_date, status } = req.body;
  if (!bull_id || !slot_id || quantity == null) {
    return res.status(400).json({ error: 'bull_id, slot_id, and quantity are required' });
  }
  try {
    ...
    const { rows } = await pool.query(
      ...
      [bull_id, slot_id, quantity, sio_batch_code, production_date, status || 'approved']
    );
